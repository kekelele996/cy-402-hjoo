...
type Router struct {
	...
	timeEntry *handler.TimeEntryHandler
...
func New(cfg *config.Config, db *gorm.DB, logger *slog.Logger,
	...
	document *handler.DocumentHandler, billing *handler.BillingHandler, timeEntry *handler.TimeEntryHandler,
	...
	return &Router{
		...
		document: document, billing: billing, timeEntry: timeEntry, upload: upload, auditLog: auditLog,
	}
}
